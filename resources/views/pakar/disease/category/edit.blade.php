...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('pakar.kategori_penyakit.update', $diseaseCat->id) }}" method="post">
                    @method('PUT')
                    @csrf
                    ...
                    <div class="form-group">
                        <label>{{ __("Nama Wabah / Penyakit / Virus") }}</label>
                        <input type="text" name="disease" class="form-control @error('disease') is-invalid @enderror" value="{{ $diseaseCat->disease->name }}" placeholder="{{ $diseaseCat->disease->name }}" readonly>
                        @error('disease')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="category" class="form-control @error('category') is-invalid @enderror" value="{{ $diseaseCat->name }}" placeholder="{{ $diseaseCat->name }}">
                        @error('category')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
